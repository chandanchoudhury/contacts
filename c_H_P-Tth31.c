...
#define NFILE asize(fnm)
    ...
    TriadsBegin = 0; TriadsEnd   = 37818;

    k = 0;
    for (i = TriadsBegin; i < TriadsEnd; i++) {
    ...
    do {
        ...
        for(i = 0; i <= ProtMax; i++) {
            for(j = 0; j <= TriadsMax; j++) {
                distsq = 0.0;
                for(d = 0; d < DIM; d++) {
                    /* Implementing the Minimum Image Distance Algorithm */
                    B[d] = fr.x[TriadsStore[j]][d];

                    if((fr.x[ProteinH[i]][d] - fr.x[TriadsStore[j]][d]) > (0.50 * fr.box[d][d]))
                        B[d] += fr.box[d][d];
                    if((fr.x[ProteinH[i]][d] - fr.x[TriadsStore[j]][d]) < -(0.50 * fr.box[d][d]))
                        B[d] -= fr.box[d][d];
