...
*/
	...
	FILE  *P_fT ;
    P_fT           = fopen("P-fTth31_0-250.xvg","wb");

    int ProteinHA[50000]={0}, SOLHA[50000]={0}, fTriadHA[50000]={0};
    int ProtMax = 0, SOLMax = 0, fTriadMax = 0;
    int tot_no_Amino_Acids = 0, d = 0, size = 99999;
    int pairsP_T = 0;
    int uniqueP_T = 0;
    int *storeP_T, count = 0;
    ...
    storeP_T = malloc(sizeof(int) * size);
    memset(storeP_T, 0, sizeof(int) * size);
    ...
    for (i = 0; i < 3988; i++) {
        ...
        for( ;tot_no_Amino_Acids > 0; ) {
            if(strcmp(*(top.atoms.resinfo[top.atoms.atom[i].resind].name), AminoAcids[--tot_no_Amino_Acids]) == 0) {
                if(strncmp(*(top.atoms.atomname[i]), "H",1) != 0) { 
                    ProteinHA[k] = i; ProtMax = k; k++;
                }
            }
        }
    }
    ...
    k = 0;
    for (i = 4018; i < 37818; i++)
        if(strncmp(*(top.atoms.atomname[i]), "H",1) != 0){
            fTriadHA[k] = i; fTriadMax = k; k++;
        }

//    printf("%d %d %d\n",ProtMax, SOLMax);

	do {
        /* Protein-fTriad contacts */
        memset(storeP_T, 0, sizeof(int) * size);
        pairsP_T = 0; uniqueP_T = 0;

        for(i = 0; i <= ProtMax; i++) {
            for(j = 0; j <= fTriadMax; j++) {
                distsq = 0.0;
                for(d = 0; d < DIM; d++) {
                    /* Implementing the Minimum Image Distance Algorithm */
                    B[d] = fr.x[fTriadHA[j]][d];

                    if((fr.x[ProteinHA[i]][d] - fr.x[fTriadHA[j]][d]) > (0.50 * fr.box[d][d]))
                        B[d] += fr.box[d][d];
                    if((fr.x[ProteinHA[i]][d] - fr.x[fTriadHA[j]][d]) < -(0.50 * fr.box[d][d]))
                        B[d] -= fr.box[d][d];

                   distsq += (pow((fr.x[ProteinHA[i]][d] - B[d]),2));
                }
                if(distsq < thP_Tsq) {
                    storeP_T[pairsP_T] = j;
                    pairsP_T++;
        ...
        /**************************************************************************/
        count = 0;
        for(j = 0; j < pairsP_T; j++) {
            count = 0;
            for(i = j; i >= 0; i--) {
                if(storeP_T[j] == storeP_T[i])
                    count++;
            }
            if(count < 2) {
                uniqueP_T++;
