...
#define thP_OW 0.440  /* Protein HA - OW. 1st solvation Minima */
...
*/
	...
	FILE  *P_OW;
    P_OW          = fopen("P-OWth44_0-250.xvg","wb");

    int ProteinHA[50000]={0}, SOLHA[50000]={0};
    int ProtMax = 0, SOLMax = 0;
    int tot_no_Amino_Acids = 20, d = 0, size = 99999;
    int pairsP_OW = 0;
    int uniqueP_OW = 0;
        ...
        for(i = 0; i <= ProtMax; i++) {
            for(j = 0; j <= SOLMax; j++) {
                ...
                for(d = 0; d < DIM; d++) {
                    ...
                    if((fr.x[ProteinHA[i]][d] - fr.x[SOLHA[j]][d]) > (0.50 * fr.box[d][d]))
                        B[d] += fr.box[d][d];
                    if((fr.x[ProteinHA[i]][d] - fr.x[SOLHA[j]][d]) < -(0.50 * fr.box[d][d]))
                        B[d] -= fr.box[d][d];
