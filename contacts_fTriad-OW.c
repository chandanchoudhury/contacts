...
*/
	...
	FILE  *fTriad_OW;
    fTriad_OW      = fopen("fT-OW250-500.xvg","wb");

    int ProteinHA[50000]={0}, fTriadHA[50000]={0}, SOLHA[50000]={0};
    int ProtMax = 0, fTriadMax = 0, SOLMax = 0;
    int tot_no_Amino_Acids = 21, d = 0, size = 99999;
    int pairsTriad_OW = 0;
    int uniqueTriad_OW = 0;
    int *storeTriad, *storeTriad_OW, count = 0;
    float dx = 0.0, half_box = 0.0;
    ...
    k = 0;
    for (i = 37818; i < 42234; i++)
        if(strncmp(*(top.atoms.atomname[i]), "H",1) != 0) {
            SOLHA[k] = i; SOLMax = k; k++;
        }

//    printf("%d %d %d\n",ProtMax, fTriadMax, SOLMax);
        ...
        for(i = 0; i <= fTriadMax; i++) {
            for(j = 0; j <= SOLMax; j++) {
                ...
                for(d = 0; d < DIM; d++) {
                    ...
                    dx = fr.x[fTriadHA[i]][d] - fr.x[SOLHA[j]][d];
                    half_box = 0.50 * fr.box[d][d];
                   ...
                   distsq += (pow((fr.x[fTriadHA[i]][d] - B[d]),2));
                }
                if(distsq < thT_OWsq) {
