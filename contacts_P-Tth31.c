...
*/
	...
	FILE  *P_T ;
    P_T           = fopen("P-Tth31_0-250.xvg","wb");

    int ProteinHA[50000]={0}, SOLHA[50000]={0}, TriadHA[50000]={0};
    int ProtMax = 0, SOLMax = 0, TriadMax = 0;
    int tot_no_Amino_Acids = 0, d = 0, size = 99999;
    int no_triad_residues = 0;
    int pairsP_T = 0;
    int uniqueP_T = 0;
...
//    printf("%d %d\n", ProtMax, TriadMax);
	do {
        /* Protein-Triad contacts */
        memset(storeP_T, 0, sizeof(int) * size);
        pairsP_T = 0; uniqueP_T = 0;

        for(i = 0; i <= ProtMax; i++) {
            for(j = 0; j <= TriadMax; j++) {
                distsq = 0.0;
                for(d = 0; d < DIM; d++) {
                    /* Implementing the Minimum Image Distance Algorithm */
                    B[d] = fr.x[TriadHA[j]][d];

                    if((fr.x[ProteinHA[i]][d] - fr.x[TriadHA[j]][d]) > (0.50 * fr.box[d][d]))
                        B[d] += fr.box[d][d];
                    if((fr.x[ProteinHA[i]][d] - fr.x[TriadHA[j]][d]) < -(0.50 * fr.box[d][d]))
                        B[d] -= fr.box[d][d];
